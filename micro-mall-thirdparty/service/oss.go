...
func (oss *OSS) GetOSSToken(ctx context.Context, req *proto_thirdparty.GetOSSTokenRequest) (*proto_thirdparty.GetOSSTokenResponse, error) {
	response := get_policy_token()
	resp := &proto_thirdparty.GetOSSTokenResponse{
		AccessId:  response.AccessKeyId,
		Host:      response.Host,
...
func InitArgs() {
	// 请填写您的AccessKeyId。
	accessKeyId = global.GVA_CONFIG.ThirdParty.AccessKeyId

	// 请填写您的AccessKeySecret。
	accessKeySecret = global.GVA_CONFIG.ThirdParty.AccessKeySecret

	// host的格式为 bucketname.endpoint ，请替换为您的真实信息。
	host = global.GVA_CONFIG.ThirdParty.Host

}

// callbackUrl为 上传回调服务器的URL，请将下面的IP和Port配置为您自己的真实信息。
var callbackUrl string = "http://127.0.0.1:8080"

// 用户上传文件时指定的前缀。
var upload_dir string = "micro-mall-micro-mall-product-proto/"
...
func get_policy_token() PolicyToken {
	now := time.Now().Unix()
